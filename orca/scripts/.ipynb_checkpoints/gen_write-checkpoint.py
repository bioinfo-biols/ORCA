...
def get_gtf_indexed(gtf_path):
    ...
    with open (gtx_path, 'r') as gtf, open(exon_path, 'w') as exon, open(index_path, 'w') as index:
            ...
            if clas == 'exon':
                ...
                if txid != old_txid:
                    if old_txid != '':
                        ...
                        index.write(f'{txid},{start},{end}\n')
                        start = end
                        exon.write(f'{line.rstrip()},{txid}\n')
                    else:
                        start = exon.tell()
                        exon.write(f'{line.rstrip()},{txid}\n')
                else:
...
def main():
    ...
    parser.add_argument('--prefix', required=True, help='Prediction result file (e.g. *.preds.per.site_212)')
    parser.add_argument('--output_path', required=True, help='DIRECTORY of output file')
    parser.add_argument('--n_processes', type=int, default=16)
    args = parser.parse_args()

    inputs = f'{args.output_path}/{args.prefix}.preds.per.site'
    outputs = f'{args.output_path}/{args.prefix}.preds.per.site.gen'
